...
from lda import create_document, create_corpus, do_LDA, do_LDA_Mallet, getDocTopicDist_mp, save_documents, \
    parallel_create_document, serializeCorpus, load_corpus, load_dictionary
...
if __name__ == "__main__":
    try:
        logging.info("++++++++++++++++++++ START LOG ENTROPY ++++++++++++++++++++")
        t1 = time.time()
        ...
        # read dictionary
        logging.info("Loading dictionary from %s ." % OUTPUT_DIR)
        dictionary_name = 'dictionary-k%s.gensim' % (''.join(str(val) for val in k))
        dictionary = load_dictionary(OUTPUT_DIR, dictionary_name)

        # read Bow corpus
        logging.info("Loading corpus from %s ." % OUTPUT_DIR)
        ...
        top_dist, lda_keys = getDocTopicDist_mp(model=lda_model, corpus=log_entropy_corpus, n_workers=n_workers,
                                                n_topics=n_topics)
